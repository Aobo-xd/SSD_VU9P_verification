...
# -------------------------------------------------------------------------------
# 功能：1，对图像进行标定
#       2，对大分辨率图像可以进行拖动进度条调整位置，从而进行标定
#       3，查看以前标记好的位置
# 作者： 王奥博
# 时间：2017-06

# 修改： zhao，ck
# 修复一些小bug，照片不能回看，修改默认种类导致第一个种类不能标记
# 时间：2017-07-08

'''
使用说明：
    将需要标定的图片放在images文件夹下（需要自己创建。如不存在，程序会报错）（可以通过修改config.txt文件中的路径名改变）
    输出txt文档为标记信息，第一行为标记数量，从第二行开始为具体标记信息。输出txt文档名与标记的图片名一样，存放在labels文件夹下（需要自己创建。如不存在，程序会报错）（可以通过修改config.txt文件中的路径名改变）
'''
# -------------------------------------------------------------------------------
...
class LabelTool():
    def __init__(self, master):
        ...
        self.ldBtn = Button(self.frame,width=15, text="bridge", command=self.bridge)  # 改成确认加上的例子
        self.ldBtn.grid(row=0, column=3, sticky=W + E)
        self.ldBtn = Button(self.frame,width=15, text="harbor", command=self.harbor)#改成确认加上的例子
        self.ldBtn.grid(row=1, column=2, sticky=W + E)
        self.ldBtn = Button(self.frame,width=15, text="plane", command=self.plane)  # 改成确认加上的例子
        self.ldBtn.grid(row=1, column=3, sticky=W + E)
        self.ldBtn = Button(self.frame,width=15, text="ship", command=self.ship)  # 改成确认加上的例子
        self.ldBtn.grid(row=2, column=2, sticky=W + E)
        self.ldBtn = Button(self.frame,width=15, text="storage", command=self.storage)  # 改成确认加上的例子
...
#你可以使用sticky选项去指定对齐方式，可以选择的值有：N/S/E/W，分别代表上/下/左/右。如果你想让label靠左显示，你可以设置stricky的值为W。
        ...
        self.frame1.grid(row=1, column=1, rowspan=5, sticky=W + N)

        self.mainPanel = Canvas(self.frame1,width=900,height=900, cursor='tcross')
        #Canvas类似于一个画板，可以在上面绘制各种图形和进行图形编辑


        vbar=Scrollbar(self.frame1)                       #与之前的一样
...
#        self.mainPanel.grid(row=1, column=1, rowspan=5, sticky=W + N)
        ...
        self.listbox.grid(row=3, column=2,columnspan=2,sticky=N)
        #显示数据

        self.btnDel = Button(self.frame, text='Delete', command=self.delBBox)
        self.btnDel.grid(row=4, column=2,columnspan=2, sticky=W + E + N)
        self.btnClear = Button(self.frame, text='ClearAll', command=self.clearBBox)
        self.btnClear.grid(row=5, column=2,columnspan=2, sticky=W + E + N)
    ...
    def loadImage(self):
        ...
        self.w2=self.w0/wh_ratio  #原始图片的长
        self.h2=self.h0/wh_ratio #原始图片的高
        print self.w2,self.h2

        fullsize=(0,0,self.w0,self.h0) #最大的大小
    ...
    def mouseClick(self, event):
        #print (self.mainPanel.canvasx(event.x),self.mainPanel.canvasy(event.y))

   #     print "self.w2  self.h2" 
    #    print self.w2 
    #    print self.h2
        if self.STATE['click'] == 0:
            if self.mainPanel.canvasx(event.x)<0: 
                event.x=0
            elif self.mainPanel.canvasy(event.y)<0:
                event.y = 0
            elif self.mainPanel.canvasx(event.x)>self.w2:
                event.x=self.w2
            elif self.mainPanel.canvasy(event.y)>self.h2:
                event.y = self.h2
    ...
    def mouseMove(self, event):
        self.disp.config(text='x: %.2f, y: %.2f' % (self.mainPanel.canvasx(event.x) / self.w2, self.mainPanel.canvasy(event.y) / self.h2))
        if self.tkimg:
            if self.hl:
                self.mainPanel.delete(self.hl)
            #self.hl = self.mainPanel.create_line(0, self.mainPanel.canvasx(event.y), self.tkimg.width(), self.mainPanel.canvasx(event.y), width=2)
